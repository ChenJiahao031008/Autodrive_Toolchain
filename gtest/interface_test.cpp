...
TEST(TestDataReader, TestGetPose)
{
    ...
    auto vPoses = interface::DataReader::GetTUMPoseFromFile(pose_stream);
    ...
    vPoses = interface::DataReader::GetTUMPoseFromFile(pose_stream2, 3);
...
TEST(TestDataReader, FolderOperation)
{
    ...
    interface::DataReader::CopyFiles(path0, path3);
    // 执行文件拷贝操作，目标文件夹不存在时会自动创建(文件->文件夹)
    interface::DataReader::CopyFiles(path0, path5);

    // mode=0:不允许递归, 输出文件和文件夹;
    interface::DataReader::ReadFolder(path5, filenames, 0);
    EXPECT_EQ(filenames[0], path5 + "lidar_pose.txt");
    filenames.clear();

    // mode=1:允许递归, 但是只输出文件名;
    interface::DataReader::ReadFolder(path1, filenames, 1);
    EXPECT_EQ(filenames[0], path3);
    filenames.clear();

    // mode=2:允许递归, 但是只输出文件夹;
    interface::DataReader::ReadFolder(path1, filenames, 2);
    EXPECT_EQ(filenames[0], path2);
    filenames.clear();

    // mode=3: 允许递归, 输出文件名和文件路径;
    interface::DataReader::ReadFolder(path1, filenames, 3);
    ...
    interface::DataReader::CopyFiles("../../gtest/config_data", path4);
    interface::DataReader::ReadFolder(path4, filenames, 1);
    sort(filenames.begin(), filenames.end());
    ...
    // 获取文件的名称，不包含路径和扩展名
    std::string res_str;
    interface::DataReader::GetFileNameInPath(path3, res_str);
