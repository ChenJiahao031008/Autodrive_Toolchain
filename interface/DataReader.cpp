...
namespace interface {
std::vector<sensorData::PoseData> DataReader::GetTUMPoseFromFile(
    std::ifstream &pose_stream, int drop_lines_num) {
  std::string line;
...
void DataReader::ReadFolder(const std::string &folder,
                            std::vector<std::string> &files, int mode) {
...
void DataReader::LoadExtrinsic(const std::string &file_path,
...
bool DataReader::LoadIntrinsic(const std::string &intrinsics_path,
                               cv::Mat &dist_coeffs, cv::Mat &intrisic_mat) {
#if GCC_VERSION >= 90400
  if (!(std::filesystem::exists(intrinsics_path))) return false;
#else
  if (!(boost::filesystem::exists(intrinsics_path))) return false;
#endif
...
bool DataReader::CopyFiles(const std::string &src, const std::string &dst,
                           int mode) {
#if GCC_VERSION >= 90400
  std::error_code ec;
#else
  ...
  if (fs::is_regular_file(src_path)) {
    // dst is not exist
    if (!fs::exists(dst_path)) fs::create_directories(dst_path.parent_path());
    ...
    // case3: src is a file, dst is neither file nor directory
      ...
      if (ec) {
        AERROR << "Copy File Failed: dst is neither file nor directory. ec: "
               << ec.message();
        return false;
      }
    }
...
#if GCC_VERSION >= 90400
        if (mode == 0) {
          fs::copy_file(newSrc, newDst, fs::copy_options::overwrite_existing,
                        ec);
        } else {
...
#else
        if (mode == 0) {
          fs::copy_file(newSrc, newDst, fs::copy_option::overwrite_if_exists,
                        ec);
        } else {
          fs::rename(newSrc, newDst, ec);
        }
...
bool DataReader::CopyDirectory(const std::string &strSourceDir,
  ...
  for (fs::recursive_directory_iterator pos(strSourceDir); pos != end; ++pos) {
    //过滤掉目录和子目录为空的情况
    if (fs::is_directory(*pos)) continue;
...
bool DataReader::GetFileNameInPath(const std::string &path,
                                   std::string &filename) {
...
std::string DataReader::GetFileExtension(const std::string &path) {
  const fs::path _path_(path);
